    ...
    if report.get_media():
        ...
        if report_photos:
            if len(report_photos) == 1:
                await context.bot.send_photo(user_id, report_photos[0], caption="Додане фото ⬆️",
                ...
                await context.bot.send_media_group(user_id, photos_list, caption="Додані фото ⬆️",
                                                   reply_to_message_id=msg_id)

        if report_docs:
            if len(report_docs) == 1:
                await context.bot.send_document(user_id, report_docs[0], caption="Доданий документ ⬆️",
                                                reply_to_message_id=msg_id)
                ...
                await context.bot.send_media_group(user_id, docs_list, caption="⬇️ Додані документи ⬆️",
                                                   reply_to_message_id=msg_id)

        if report_videos:
            if len(report_videos) == 1:
                await context.bot.send_video(user_id, report_videos[0], caption="Додане відео ⬆️",
                                             reply_to_message_id=msg_id)
                ...
                await context.bot.send_media_group(user_id, videos_list, caption="Додані відео ⬆️",
                                                   reply_to_message_id=msg_id)
...
async def print_daily_reports(update: Update, context: ContextTypes.DEFAULT_TYPE, daily_reports) -> None:
    for report in daily_reports:
        delete_button = [
            [InlineKeyboardButton("❌ Видалити", callback_data=f"delete_daily_{report.report_id}")]
        ]
        msg = await update.message.reply_text(report.create_report_text(),
                                              reply_markup=InlineKeyboardMarkup(delete_button))
        await send_media(report, config.ADMIN_ID, msg.id, context)
...
async def print_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE, tasks) -> None:
    for task in tasks:
        ...
        await send_media(task, config.ADMIN_ID, msg.id,  context)
...
async def print_reports(update: Update, context: ContextTypes.DEFAULT_TYPE, reports) -> None:
    for report in reports:
        if report.report_status == "pending":
            ...
            delete_button = [
                [InlineKeyboardButton("❌ Видалити", callback_data=f"delete_report_{report.report_id}")]
            ]
